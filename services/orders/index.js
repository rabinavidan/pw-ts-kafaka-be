...
async function publish(topic, key, value, headers = {}) {
  ...
  try {
    ...
    console.warn(`[orders] Failed to publish to ${topic}:`, err.message);
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (method === 'GET' && path === '/api/v1/orders') {
    ...
    const rowsQ = statusFilter
      ? await pool.query(`SELECT * FROM orders WHERE status = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
          [statusFilter, pageSize, offset])
      : await pool.query(`SELECT * FROM orders ORDER BY created_at DESC LIMIT $1 OFFSET $2`, [pageSize, offset]);
...
async function start() {
  await initDB();
  producer.connect()
    .then(() => { producerReady = true; console.log('[orders] Kafka producer connected'); })
    .catch(err => console.warn('[orders] Kafka unavailable:', err.message));
  server.listen(PORT, () => console.log(`[orders] Listening on http://localhost:${PORT}`));
}
